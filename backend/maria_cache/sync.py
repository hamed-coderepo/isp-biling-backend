...
def _replace_for_source(model, source_name, rows, builder, db_alias='cache'):
    model.objects.using(db_alias).filter(source_name=source_name).delete()
    if not rows:
        return 0
    items = [builder(row) for row in rows]
    model.objects.using(db_alias).bulk_create(items, ignore_conflicts=True)
    return len(items)


def sync_reference_tables(source_name=None):
